...
llm_llava = OllamaLLM(model="llava:7b", 
    temperature=0,)
...
chroma_cache_vector = Chroma(persist_directory="./src/vector_dbs/cache_vector_db", embedding_function=sntnc_trnsfrmr_embeddings, collection_metadata={"hnsw:space": "cosine"})
...
cache_docstore = LocalFileStore("./src/vector_dbs/cache_docs")
...
def cache_update(question, answer):
    ...
    whole_docs.append((unique_id, answer.encode('utf-8')))
    vector_store_docs.append(Document(page_content=question, metadata={"doc_id": unique_id}))
...
def cache_retrival(question):
    ...
    if rlvnt_df.empty:
        ...
        try:
            ...
            answer = cache_docstore.mget([cache_id])
            return answer[0].decode('utf-8')
        except:
            return None    
...
def load_and_split_document(file_path: str) -> List[Document]:
    ...
    for conv_res in conv_results:
        ...
        with (output_dir / f"{doc_filename.replace(" ","_")}.md").open("w", encoding="utf-8") as fp:
            fp.write(conv_res.document.export_to_markdown())
        print("document processed")
        for tbl_idx, tbl_i in enumerate(conv_res.document.tables):
            tables.append(tbl_i.export_to_markdown())
            table_counter += 1
        print("tables processed")
        for txt_idx, txt in enumerate(conv_res.document.texts):
            if txt.label != 'section_header':
                ...
                not_prcsd.append([txt_idx, txt])
        print("texts processed")
        os.makedirs(f"{output_dir}/images/", exist_ok=True)
        for img_idx, element in enumerate(conv_res.document.pictures):
            # break
    ...
    print("table_summaries processed")
    text_summaries = summarize_chain.batch(texts, {"max_concurrency": 3})
    print("text_summaries processed")
    ...
    for img_idx, image_b64 in enumerate(img_base64_list):
        retries = 3
        success = False
        while retries > 0 and not success:
            print(f"Retries left: {retries}", f"Processing image {img_idx}")
            def target():
                try:
    ...
    return all_splits_pypdf_texts, text_summaries, texts, table_summaries, tables, image_summaries, img_base64_list
...
def create_multi_vector_retriever(
    ...
    if image_summaries:
        print("Adding images")
        print(f"len_image_summaries: {len(image_summaries)}, len_images: {len(images)}")
...
def index_document_to_chroma(file_path: str, file_id: int) -> bool:
    ...
    try:
        all_splits_pypdf_texts, text_summaries, texts, table_summaries, tables, image_summaries, img_base64_list  = load_and_split_document(file_path)
