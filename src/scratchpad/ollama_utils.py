...
Core Commands:
- ollama run <model>: Starts a model session and allows interaction. Downloads model if not locally installed.
- ollama pull <model>: Downloads a model from the Ollama model library to local system.
- ollama list: Displays all locally downloaded models.
- ollama ps: Shows currently running Ollama processes.
- ollama create <name> -f <Modelfile>: Creates a custom model based on a Modelfile.

Other Useful Commands:
- ollama serve: Starts the Ollama server for API access.
- ollama show <model>: Displays details about a specific model (e.g., parameters).
- ollama rm <model>: Deletes a model from local storage.
- ollama cp <source> <destination>: Copies a model.
- ollama push <model>: Uploads a model to a registry.
- ollama stop <process_id>: Stops a running Ollama process.
- ollama help or ollama --help: Shows help information for Ollama commands.
"""
